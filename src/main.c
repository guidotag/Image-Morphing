...
void morph(IplImage *src_image, IplImage *dst_image, segment *src_segments, segment *dst_segments, int n_segments, int n_frames, CvVideoWriter *writer);
...
CvVideoWriter *create_video_writer (const char *output_file, int width, int height, int fps) {
	...
	if(writer == NULL) {
        fprintf(stderr, "Null video writer\n");
        exit(EXIT_FAILURE);
    }
